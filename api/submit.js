export default async (req, res) => {
  ...
  try {
    ...
    // 1) Redis: pending
    const r1 = await fetch(`${process.env.UPSTASH_REDIS_REST_URL}/set/${sid}/pending`, {
      headers: { Authorization: `Bearer ${process.env.UPSTASH_REDIS_REST_TOKEN}` }
    });
    if (!r1.ok) {
      const t = await r1.text().catch(()=>'');
      return res.status(500).json({ ok:false, error:'redis_set_failed', detail:t });
    }

    // 2) Discord channel message with buttons
    ...
    const r2 = await fetch(`https://discord.com/api/v10/channels/${process.env.DISCORD_CHANNEL_ID}/messages`, {
      method: 'POST',
      headers: {
