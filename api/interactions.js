...
export default async (req, res) => {
  try {
    ...
    if (i.type === 3) { // BUTTON INTERACTION
      const [action, sid] = i.data.custom_id.split(':');
      const state = action === 'approve' ? 'approved' : 'rejected';

      const r = await fetch(`${process.env.UPSTASH_REDIS_REST_URL}/set/${sid}/${state}`, {
        headers: { Authorization: `Bearer ${process.env.UPSTASH_REDIS_REST_TOKEN}` }
      });
      if (!r.ok) {
        return res.json({ type: 4, data: { content: 'Redis hatası', flags: 64 } });
      }
